...
document.addEventListener('DOMContentLoaded', function() {
    ...
    physicsDropBtn.addEventListener('click', function() {
        ...
        if (physicsEnabled) {
            ...
            // Store original positions and remove animations, add physics
            clickableObjs.forEach((obj, index) => {
                ...
                // Add physics body with slight delay for effect
                setTimeout(() => {
            ...
            physicsInfo.className = 'section-title';
            physicsInfo.innerHTML = 'Physics Mode: Click objects to push them!';
            if (!document.getElementById('physics-info')) {
                infoPanel.appendChild(physicsInfo);
            }
        } else {
            this.classList.remove('active');
    ...
    // Make physics objects pushable when clicked
    document.addEventListener('click', function(e) {
        if (!physicsEnabled) return;

        const clickedEl = e.target;
        if (clickedEl && clickedEl.hasAttribute && clickedEl.hasAttribute('dynamic-body')) {
            const body = clickedEl.body;
            if (body) {
                ...
                // Apply impulse in camera direction using CANNON.js
                ...
                playSound(200 + Math.random() * 300, 0.15, 'triangle');
            }
        }
    });
    ...
    addObjectBtn.addEventListener('click', function() {
        ...
        // If physics mode is enabled, add physics to spawned object
        if (physicsEnabled) {
            setTimeout(() => {
                newObj.setAttribute('dynamic-body', {
                    mass: 1,
                    linearDamping: 0.3,
                    angularDamping: 0.3
                });

                // Remove animations for physics objects
